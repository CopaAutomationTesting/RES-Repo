...
public class Regression_PROD_07 extends ATOflowPageObjects{
	...
	public void SearchPassenger(WebDriver driver, BFrameworkQueryObjects queryObjects)throws Exception{
		...
		try{
			...
			if( sSubmit.equalsIgnoreCase("Y")) {				
				...
				if(isenabled)
				{
					queryObjects.logStatus(driver, Status.PASS, "Checking Secondry document need to submit or BYPASS the doc check ", "Secondary document is mandatory", null);
           ...
           //////////////////////////this test case to validate ADC NOT OK VERIFICATION////////
				
				if(driver.findElements(By.xpath(AdcSataus)).size()>0)
				{
					queryObjects.logStatus(driver, Status.PASS, "Checking doc check ", "Doc check is unsuccessfull , This case should show the doc check as no ok", null);	
					
					
					
					//we go for adc bypass
					FlightSearch.loadhandling(driver);
					...
					driver.findElement(By.xpath(SubmitXpath)).click();
					FlightSearch.loadhandling(driver);
					Boolean adcbypass=driver.findElement(By.xpath(AdcBypass)).isDisplayed();
					if(adcbypass)
					{
						
						queryObjects.logStatus(driver, Status.PASS, "checking the adc bypass message ", "adc bypass message is display", null);
					}
					else
					{
						queryObjects.logStatus(driver, Status.FAIL, "checking the adc bypass message", "adc bypass message is not display", null);	
					}
					
				}
				else
				{
					queryObjects.logStatus(driver, Status.FAIL, "Checking doc check ", "Doc check is successfull , This case should show the doc check as no ok ", null);		
				}
	...
	public static  void EnterDocDetails(WebDriver driver,BFrameworkQueryObjects queryObjects,String fName,String sName,String PaxAge,String DType,String DNum, String Expdays,String COI,String Nty,String COR) throws IOException {
		...
		try {
			...
			if (Expdays.contains("/")) {//Added by Jenny)
				...
				calen.add(Calendar.DATE, Integer.parseInt(Expdays));
