...
public class Regression_PROD_06 extends ATOflowPageObjects{
	...
	public void SearchPassenger(WebDriver driver, BFrameworkQueryObjects queryObjects)throws Exception{
		...
		try{
			...
			String	PnrStored = Login.PNRNUM.trim();
				
				
			Boolean pnrgreenstatus=driver.findElements(By.xpath("//div[div[span[contains(text(),'"+PnrStored+"')]]]//preceding-sibling::div//i[contains(@class,'icon-checked-in active-state')]")).size()>0;
			
				
			   if(pnrgreenstatus)
			   {
				   queryObjects.logStatus(driver, Status.PASS, "Search GREEN symbol of check in PNR", "Green symol found for checkin pnr", null);
			   }
			   else
			   {
				   queryObjects.logStatus(driver, Status.FAIL, "Search GREEN symbol of check in PNR", "Green symol not found for checkin pnr ", null);  
			   }
				
			   
			   driver.findElement(By.xpath("//div[div[span[contains(text(),'"+PnrStored+"')]]]//preceding-sibling::div/md-checkbox")).click();
			   ...
			   driver.findElement(By.xpath("//button[contains(text(),'Off Load')]")).click();
				...
				driver.findElement(By.xpath(OrderNumXpath)).sendKeys(PnrStored);
				Thread.sleep(1000);
				//proceed to check in
				driver.findElement(By.xpath(SearchXpath)).click();
				FlightSearch.loadhandling(driver);
			  ...
			  /// driver.findElement(By.xpath("//button[@aria-label='Proceed to checkin']"));
			...
			//BELOW IF CONDITION IS ONLY FOR THIS PARTICULAR CASE 
			if (FlightSearch.getTrimTdata(queryObjects.getTestData("SecDocVerify")).equalsIgnoreCase("Y")) {
				...
				try
				{
				 btnenabled = driver.findElement(By.xpath(SubmitXpath)).isEnabled();	
				}
				catch(Exception e){}
				if (btnenabled) {
			...
			if( sSubmit.equalsIgnoreCase("Y")) {				
				...
				try {
					boolean	ADCstatus = driver.findElement(By.xpath(AdcOk)).isDisplayed();
					boolean	APISstatus = driver.findElement(By.xpath(APIScompletevar)).isDisplayed();
					
					if(ADCstatus && APISstatus)
					{
						queryObjects.logStatus(driver, Status.PASS,"Checking APIS AND ADC  status","APIS and ADC has completed",null);
					}
					else
					{
						queryObjects.logStatus(driver, Status.FAIL,"Checking APIS AND ADC status","APIS and ADC has not complete",null);
					}
				...
				try {
					ErrDisplayed =  driver.findElements(By.xpath(ErrXpath)).size()>0;
					...
					///for short checkin
					if(driver.findElements(By.xpath("//md-select[@ng-model='passengerData.ShortCheckAirportCode']")).size()>0)
					{
						try {
				...
				if (confirmok) {
         queryObjects.logStatus(driver, Status.PASS, "Checkin butoon search", "Checkin button is   enable as expected " , null); 
	...
	public static void EnterSecDoc(WebDriver driver, BFrameworkQueryObjects queryObjects) throws IOException{
		...
		try{   
			...
			String name=driver.findElement(By.xpath(SecSurname)).getAttribute("value");
	...
	/*	if(isSecDoc && (IssecFName!=""))
		...
		if( FLAG) {	 
			...
			queryObjects.logStatus(driver, Status.FAIL,"Checking APIS status","APIS has not complete",null);
